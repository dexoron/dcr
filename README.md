...
```sh
git clone https://github.com/dexoron/dcr.git "~/.local/share/dcr"
chmod +x "~/.local/share/dcr/dcr"
ln -sf ~/.local/share/dcr/dcr ~/.local/bin
```
...
`dcr run` или `dcr run --release` - для сбрки с оптимизацией
